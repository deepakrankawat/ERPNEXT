(() => {
	...
	function overviewSection(data) {
		...
		const quick = [
			...
			["documents", "Documents", "Upload and retrieve files", "file"],
		].filter(Boolean);
	...
	function jobTable(rows) {
		...
		return `<div class="lex-table-wrap"><table class="lex-table table"><thead><tr><th>Work item</th><th>Matter</th><th>Priority</th><th>Due</th><th>Status</th></tr></thead><tbody>${rows.map((row) => `<tr><td><strong>${escapeHTML(row.job_title)}</strong><br><small>${escapeHTML(row.name)}</small></td><td>${escapeHTML(row.engagement)}</td><td><span class="lex-pill indicator-pill ${indicatorColor(row.priority)} ${statusClass(row.priority)}">${escapeHTML(row.priority)}</span></td><td>${escapeHTML(fmtDate(row.due_date))}</td><td><span class="lex-pill indicator-pill ${indicatorColor(row.job_status)} ${statusClass(row.job_status)}">${escapeHTML(row.job_status)}</span></td></tr>`).join("")}</tbody></table></div>`;
	}
	...
	function workIntakeSection(data) {
		...
		return `<section class="lex-section" data-panel="new-matter">${sectionHeader("Submit New Work", "SLA → documents → analysis → quote → funding → activated Matter and Job")}${form}${intakeCards || card("Current intake", "No intake submitted", empty("Submit preliminary details to begin."))}</section>`;
	}

	function intakeCard(intake, data) {
		const docs = (intake.documents || []).map((row) => `<li><span><strong>${escapeHTML(row.file_name)}</strong><small>${formatBytes(row.file_size)}</small></span><span class="indicator-pill ${indicatorColor(row.custom_lex_scan_status)} ${statusClass(row.custom_lex_scan_status)}">${escapeHTML(row.custom_lex_scan_status || "Pending")}</span></li>`).join("");
		let action = "";
		if (intake.status === "SLA Pending") {
			action = `<div class="lex-intake-action"><h4>2. Review and accept SLA Document</h4>${intake.sla_document_snapshot ? `<a class="lex-button secondary btn btn-default btn-sm" href="${escapeHTML(intake.sla_document_snapshot)}" target="_blank" rel="noopener">Open SLA PDF</a>` : ""}<div class="lex-sla-document">${escapeHTML(intake.sla_terms_snapshot).replace(/\n/g, "<br>")}</div><label class="lex-check"><input type="checkbox" data-sla-check="${escapeHTML(intake.name)}"> I reviewed SLA ${escapeHTML(intake.sla_version)} and agree to this exact snapshot (SHA-256 ${escapeHTML(intake.sla_snapshot_hash.slice(0, 12))}…).</label><button class="lex-button btn btn-primary btn-sm" type="button" data-accept-sla="${escapeHTML(intake.name)}">Accept SLA and unlock upload</button></div>`;
		} else if (["Documents Pending", "Security Review", "Analysis Pending"].includes(intake.status)) {
			action = `<div class="lex-intake-action"><h4>3. Upload documents and detailed instructions</h4><p class="text-muted">Upload unlocked ${intake.sla_accepted_on ? `after acceptance on ${escapeHTML(fmtDate(intake.sla_accepted_on))}` : ""}. Every file remains quarantined until its security scan is clean.</p><form class="lex-form lex-intake-instructions" data-intake-instructions="${escapeHTML(intake.name)}"><label class="wide">Detailed instructions<textarea name="detailed_instructions" required placeholder="Add document-specific instructions, exclusions, counterparties and expected format.">${escapeHTML(intake.detailed_instructions || "")}</textarea></label><div class="wide"><button class="lex-button secondary btn btn-default btn-sm" type="submit">Save detailed instructions</button></div></form><form class="lex-form lex-intake-upload" data-intake-upload="${escapeHTML(intake.name)}"><label class="wide">Document<input class="lex-file-input" name="file" type="file" accept=".pdf,.doc,.docx,.txt,.csv,.png,.jpg,.jpeg" required></label><div class="wide lex-button-row"><button class="lex-button btn btn-primary btn-sm" type="submit">Upload and scan</button>${intake.document_count && intake.detailed_instructions ? `<button class="lex-button secondary btn btn-default btn-sm" type="button" data-analyze-intake="${escapeHTML(intake.name)}">Run extraction and analysis</button>` : ""}</div></form>${docs ? `<ul class="lex-intake-files">${docs}</ul>` : empty("No documents uploaded yet.")}</div>`;
		} else if (intake.status === "Operations Review") {
			action = `<div class="lex-intake-action lex-review-state"><h4>4. Legal Operations review</h4>${intake.pricing_rejection_reason ? `<div class="lex-alert">The proposed price was sent back for changes: ${escapeHTML(intake.pricing_rejection_reason)}</div>` : ""}<p>Analysis confidence is ${fmtNumber(intake.analysis_confidence)}%. A human reviewer must confirm scope, LexPoints, fixed quote and delivery timeline before payment is enabled.</p>${intake.analysis_summary ? `<small>${escapeHTML(intake.analysis_summary)}</small>` : ""}</div>`;
	...
	function documentsSection(data) {
		...
		return `<section class="lex-section" data-panel="documents">${sectionHeader("Documents", "Secure files from your Work Intakes, Matters and Jobs")}<article class="lex-commercial-note"><strong>Upload gate enforced</strong><span>New client documents are uploaded inside Submit New Work only after the SLA is accepted.</span><button class="lex-button secondary btn btn-default btn-sm" type="button" data-go="new-matter">Open Work Intake</button></article>${card("Document library", `${data.documents.length} files`, documentTable(data.documents))}</section>`;
	}

	function documentTable(rows) {
		if (!rows.length) return empty("No documents are available yet.");
		return `<div class="lex-table-wrap"><table class="lex-table table"><thead><tr><th>Document</th><th>Linked record</th><th>Size</th><th>Updated</th><th></th></tr></thead><tbody>${rows.map((row) => `<tr><td><strong>${escapeHTML(row.file_name)}</strong><br><small>${row.is_private ? "Private" : "Shared"}</small></td><td>${escapeHTML(row.attached_to_name)}<br><small>${escapeHTML(row.attached_to_doctype)}</small></td><td>${formatBytes(row.file_size)}</td><td>${escapeHTML(fmtDate(row.modified))}</td><td><a class="lex-button secondary btn btn-default btn-sm" href="${escapeHTML(row.file_url)}" target="_blank" rel="noopener">Open</a></td></tr>`).join("")}</tbody></table></div>`;
	}
	...
	function approvalsSection(data) {
		...
		const body = data.approvals.length ? `<div class="lex-approval-list">${data.approvals.map((row) => `<article class="lex-approval frappe-card" data-approval="${escapeHTML(row.name)}"><div class="lex-approval-head"><div><h3>${escapeHTML(row.job_title)}</h3><p>${escapeHTML(row.name)} · ${escapeHTML(row.engagement)}</p></div><span class="lex-pill indicator-pill orange ready-for-delivery">Ready for Delivery</span></div><textarea class="form-control" data-approval-notes placeholder="Decision notes (required when requesting changes)"></textarea><div class="lex-button-row"><button class="lex-button btn btn-primary btn-sm" type="button" data-decision="Approved">Approve</button><button class="lex-button danger btn btn-danger btn-sm" type="button" data-decision="Changes Requested">Request changes</button></div></article>`).join("")}</div>` : empty("No deliverables are waiting for your approval.");
	...
	function bindForms(root, data) {
		...
		root.querySelectorAll("[data-analyze-intake]").forEach((button) => button.addEventListener("click", async () => {
			...
			try {
				const result = await call("lex.work_intake.analyze_documents", { intake: button.dataset.analyzeIntake });
				notify(result.status === "Operations Review" ? "Sent to Legal Operations review" : "Analysis and quote completed", result.status === "Operations Review" ? "orange" : "green");
				reloadSection("new-matter");
			} catch (error) { showError("Analysis could not start", error); button.disabled = false; button.textContent = "Run extraction and analysis"; }
		}));
